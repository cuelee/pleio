...
## Update
## Aug 08 2025: Improved allele mismatch handling in check_sumstats().
...
class generate_sumstat_data(object):
    ...
    def check_sumstats(self):
        ...
        def check_allele_mismatch(df, label):
            res = ~df.eq(df.iloc[:, 0], axis=0).all(axis=1)
            mismatched_snps = df.index[res].to_list()
            if mismatched_snps:
                log.log('Excluded {} SNPs due to {} allele mismatch.'.format(len(mismatched_snps), label))
            return mismatched_snps
        ...
        # Check mismatches and exclude them
        mismatched_a1 = check_allele_mismatch(A1, 'A1')
        mismatched_a2 = check_allele_mismatch(A2, 'A2')
        mismatched = set(mismatched_a1).union(mismatched_a2)
        if mismatched:
            for trait in self.data.info.index:
                self.sumstats[trait].drop(index=mismatched, inplace=True)
            self.common_snps = np.setdiff1d(self.common_snps, list(mismatched))
        else:
