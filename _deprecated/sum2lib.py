...
parser.add_argument('--ncores', default=-1, type=int,
	help='Number of cores for parallel computing. Default value is -1, indication of all CPU cores available.')
...
def parse_parallel(args):
    if args.parallel == True and args.ncores == -1:
        cores = mp.cpu_count();
...
def sum2lib(input_dic, args, log):
    ...
    for i in range(len(fs)):
        if os.path.exists(fs[i]):
            prev = input_dic[fs[i]];
            fn = fs[i];
        fe = fn.split('/')[-1].split['.'][0]
        log.log('Generate output file : {fe}'.format(fe=fe));
        ...
        df = df.dropna(axis = 0, how='any', subset=[signed,'SE'])
        NF = Nrows - df.shape[0];
        log.log('sum2lib failed to converged {} SNPs'.format(NF))
        if args.gzip:
            df.to_csv(args.out+fe+'.txt.gz', index = True, sep = '\t', compression = 'gzip')
        else:
            df.to_csv(args.out+fe+'.txt', index = True, sep = '\t')
    return(0)
